...
(function() {
  $(function() {
    var backgroundResize, fullscreenFix;
...
(function() {
  $(function() {
    ...
    $('#test').on('slideStop', function(newValue) {
      $.fn.fullpage.moveTo("page", newValue.value);
    });
    $('.cd-nav-trigger').on('click', function() {
      return $('#cd-main-nav, .cd-nav-trigger').toggleClass('is-visible');
    });
    ...
    $('#fullpage').fullpage({
      ...
      afterSlideLoad: function(anchorLink, index, slideAnchor, slideIndex) {
        return $("#test").slider('setValue', slideAnchor);
      }
    });
    return $("form.new_entry").on("ajax:success", function(event, data, status, xhr) {
      return $('#ajax-modal-body').html('<p style="margin-bottom:40%;">Ваша заявка отправлена. Спасибо!</p>');
    });
  });
  ...
  $(window).on("hashchange", function() {
    if (window.location.hash.match(/\d/g)) {
      ...
      return $('.issue-date').hide();
    } else {
      $('.logo').removeClass('minify');
      return $('.issue-date').show();
    }
  });
